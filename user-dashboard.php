...
try {
    if ($user_role === 'admin') {
        // Admin sees all scan history
        $stmt = $pdo->prepare("SELECT sh.id, sh.filename, sh.status, sh.created_at, sh.critical, sh.high, sh.medium, u.username 
        ...
        $stmt = $pdo->prepare("SELECT sh.id, sh.filename, sh.status, sh.created_at, sh.critical, sh.high, sh.medium 
                               FROM scan_history sh 
                               WHERE sh.user_id = ? 
                               ORDER BY sh.created_at DESC");
